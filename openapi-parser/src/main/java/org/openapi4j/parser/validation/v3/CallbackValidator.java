...
class CallbackValidator extends ExpressionValidator<Callback> {
  ...
  public void validate(OpenApi3 api, Callback callback, ValidationResults results) {
    if (callback.is$ref()) {
      validateReference(api, callback.get$ref(), results, $REF, CallbackValidator.instance(), Callback.class);
    } else {
  ...
  void validateWithOperation(OpenApi3 api, Operation operation, String expression, ValidationResults results) {
    ...
    while (matcher.find()) {
      if (!checkRequestParameter(api, operation, matcher.group(1), CALLBACKS, results)) {
        if (!checkResponseParameter(api, operation, matcher.group(1), CALLBACKS, results)) {
